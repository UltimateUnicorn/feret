...
impl Waypoint {
    ...
    fn convert(&mut self, mut chemin: &File) {
        ...
        writeln!(chemin, "{};{}", self.latitude_string, self.longitude_string).expect("Erreur d'écriture dans le fichier");
    }
}
...
fn main() -> std::io::Result<()> {
    ...
    // File opening
    if let Ok(lines) = read_lines("route.rtz") {
        println!("Ouverture du fichier \"route.rtz\" : ok");
        println!("-------------------------");
            ...
            if let Ok(ligne) = line {
                ...
                if etape == 1 && re_waypoint_coordinates.is_match(&ligne) {
                    ...
                    for cap in re_waypoint_coordinates.captures_iter(&ligne) {
                        ...
                        test.latitude = cap["latitude"].parse().unwrap();
                        test.longitude = cap["longitude"].parse().unwrap();
                    }
                }

                if etape == 2 && re_waypoint_end.is_match(&ligne) {
                    etape = 0;
                    waypoints.push(test.clone());
                    println!("-------------------------");
                }
            }
        }
    ...
    println!("Export des waypoints dans le fichier \"route.csv\"");
    println!("-------------------------");
    let fichier_export = OpenOptions::new()
        .append(true)
        .create(true)
        .open("route.csv")
        .expect("Erreur : impossible d'écrire dans le fichier \"route.csv \"");
